...
import (
	...
	"github.com/cnolan3/WalkieTalkieLSP/analysis"
	"github.com/cnolan3/WalkieTalkieLSP/lsp"
	"github.com/cnolan3/WalkieTalkieLSP/rpc"
)

func main() {
	logger := getLogger("/home/connor/Documents/WalkieTalkieLSP/log.txt")
...
func handleMessage(logger *log.Logger, writer io.Writer, state analysis.State, method string, contents []byte) {
	logger.Printf("Receive msg with method: %s", method)

	switch method {
	case "initialize":
		var request lsp.InitializeRequest
		if err := json.Unmarshal(contents, &request); err != nil {
			logger.Printf("Hey, we coulnt parse this: %s", err)
		}

		logger.Printf("Coneected to: %s %s", request.Params.ClientInfo.Name, request.Params.ClientInfo.Version)
...
func getLogger(filename string) *log.Logger {
	...
	return log.New(logfile, "[WalkieTalkieLSP]", log.Ldate|log.Ltime|log.Lshortfile)
}
